...
def loess_query(x_query, X, y, alpha):
  ...
  X = np.insert(X, obj=0, values=1, axis=1) # insert value behind the ojb th element
...
def weights_matrix(x_query, X, alpha):
  ...
  W = np.identity(m)        # skeleton for weights matrix, diag[1,1,...1]
...
# if W[i][i] < 1, assignment with new value, else set 0
